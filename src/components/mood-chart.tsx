...
export function MoodChart({
  ...
  const renderLineChart = () => {
    ...
    const chartWidth = Math.max(data.length * pointSpacing, 280);
    const dotSize = 8;
    ...
    return (
      <ScrollView
        ...
        <View style={{ width: chartWidth, height: maxChartHeight + 50, position: 'relative' }}>
          {/* Grid lines */}
          <View style={styles.lineChartGridContainer}>
          ...
          {data.map((item, index) => {
            ...
            const prevScore = prev.score > 0 ? prev.score : 0;
            const currScore = item.score > 0 ? item.score : 0;
            if (prevScore === 0 && currScore === 0) return null;

            const prevX = index * pointSpacing - pointSpacing + pointSpacing / 2;
            const currX = index * pointSpacing + pointSpacing / 2;
            const prevY = maxChartHeight - (prevScore / 5) * maxChartHeight;
            const currY = maxChartHeight - (currScore / 5) * maxChartHeight;
            ...
            return (
              <View
                ...
                style={{
                  position: 'absolute',
                  left: prevX + dotSize / 2 - 1,
                  top: prevY + dotSize / 2 - 1.5,
                  width: length,
                  height: 3,
          ...
          {data.map((item, index) => {
            ...
            const yPos = hasScore
              ? maxChartHeight - (item.score / 5) * maxChartHeight
              : maxChartHeight;
            const xPos = index * pointSpacing + pointSpacing / 2;

            let dotColor = '#EAE6E1';
            if (item.moodId && MOOD_MAP[item.moodId]) {
              dotColor = MOOD_MAP[item.moodId].color;
            ...
            return (
              <Pressable
                ...
                {isSelected && (
                  <View style={[styles.tooltip, { top: -28 }]}>
                ...
                <View
                  style={{
                    ...
                    borderRadius: (isSelected ? dotSize + 4 : dotSize) / 2,
            ...
            <View style={styles.chartToggleRow}>
              <Pressable
                ...
                <Ionicons
                  name="bar-chart"
                  size={15}
                  color={chartType === 'bar' ? '#FFFFFF' : '#6F4E37'}
                />
              </Pressable>
              <Pressable
                style={[
                ...
                <Ionicons
                  name="trending-up"
                  size={15}
                  color={chartType === 'line' ? '#FFFFFF' : '#6F4E37'}
                />
              </Pressable>
            </View>
          )}
